 ...
 */	
	...
	public function authenticate(&$ps_username, $ps_password="", $pa_options=null) {
        	...
        	if($pa_options['logintype'] === "surfnet"){
	            //coming from collective access login screen with 'surfnet' selected as login type
		    ...
		    $user_data = unserialize(end(explode($this->_CONFIG->get('surfnet_user_key'), $ps_password))); // get user data sent from simplesaml authenticator script
                ...
                if(!$this->load($ps_username)){ // check if user exists, user has already been successfully authenticated by surfnet
                    ...
                    if($user_created){
                        ...
                        //Display message
						$vs_message = _t("Your account needs to be activated by the system administrator. You will be notified via email once your account has been activated.<br>");
